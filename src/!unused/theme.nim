...
proc mkPublicName(name: string): NimNode =
  nnkPostfix.newTree(newIdentNode("*"), newIdentNode(name))

proc mkProperty(name: string, typ: NimNode): NimNode =
  nnkIdentDefs.newTree(mkPublicName(name), typ, newEmptyNode())

proc mkRefObjectTypeDef(name: string, recList: NimNode): NimNode =
  nnkTypeDef.newTree(
    mkPublicName(name),
    newEmptyNode(),
    nnkRefTy.newTree(nnkObjectTy.newTree(newEmptyNode(), newEmptyNode(), recList)),
  )

proc makeStyleTypeDef(name: string, props: seq[(string, NimNode)]): NimNode =
  var recList = nnkRecList.newTree
  for propName, propType in props:
    recList.add(mkProperty(propName, propType))

  result = mkRefObjectTypeDef(styleClassName(name), recList)

proc handleProperty(
    prop: NimNode, configSectionName: string, sectionNameSym, subsectionNameSym: NimNode
  ...
  if propType.kind == nnkIdent:
    ...
    of "string", "bool", "Color", "float":
      ...
      writeThemeFrag.add quote do:
        result.setSectionKey(`configSectionName`, `propName`, $`fromProp`)
    else: # enum
      parseThemeFrag.add quote do:
        getEnum[`propType`](config, `configSectionName`, `propName`, `toProp`)
    ...
    for i in 1 .. numColors:
      ...
      parseThemeFrag.add quote do:
        config.getColor(`configSectionName`, `propNameN`, `toProp`[`index`])

      writeThemeFrag.add quote do:
        result.setSectionKey(`configSectionName`, `propNameN`, $`fromProp`[`index`])
  else:
  ...
  for section in arg:
    ...
    for subsection in section[1]:
      ...
      for prop in propsList:
        let (propName, propType, parseThemeFrag, writeThemeFrag) =
          handleProperty(prop, configSectionName, sectionNameSym, subsectionNameSym)
        propsToAdd.add((propName, propType))
        parseThemeBody.add(parseThemeFrag)
        writeThemeBody.add(writeThemeFrag)

      typeSection.add(makeStyleTypeDef(prefixedSubsectionName, propsToAdd))
    ...
    typeSection.add(makeStyleTypeDef(sectionName, sectionPropsToAdd))

    themeStyleRecList.add(
      mkProperty(sectionName, newIdentNode(styleClassName(sectionName)))
    )

  typeSection.add(mkRefObjectTypeDef("ThemeStyle", themeStyleRecList))
...
proc loadThemeHocon*(filename: string): HoconNode =
  ...
  cfg.limit(
    "level.background-hatch.spacing-factor", LevelBackgroundHatchSpacingFactorLimits
  )
