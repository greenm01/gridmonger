...
proc eatRune(s): Rune =
  if s.peekBuf.len > 0: s.peekBuf.popFirst else: s.readNextRune
...
proc raiseTokeniserError(
    t; msg, details: string, line = t.line, column = t.column
) {.noReturn.} =
  t.scanner.close
  raise newException(
    HoconTokeniserError, fmt"{msg} at line {line}, column {column}: {details}"
  )
...
proc readEscape(t; line, col: Natural): Rune =
  ...
  of Rune('u'):
    ...
    t.raiseTokeniserError(
      ...
      details = fmt"\{rune} (\u{rune.ord:04x})",
      line = line,
      column = col,
    )
...
proc readUnquotedStringOrBooleanOrNull(t): Token =
  ...
  while true:
    str &= rune
    rune = t.peekRune
    if rune == Rune('\n') or rune in whitespaceRunes or rune in forbiddenRunes:
      break
    else:
      rune = t.eatRune
...
proc readNextToken(t): Token =
  ...
  case rune
  of Rune('{'):
    t.mkSimpleToken(tkLeftBrace)
  of Rune('}'):
    t.mkSimpleToken(tkRightBrace)
  of Rune('['):
    t.mkSimpleToken(tkLeftBracket)
  of Rune(']'):
    t.mkSimpleToken(tkRightBracket)
  of Rune(','):
    t.mkSimpleToken(tkComma)
  of Rune(':'):
    t.mkSimpleToken(tkColon)
  of Rune('='):
    t.mkSimpleToken(tkEquals)
  of Rune('\n'):
...
proc eatToken(t): Token =
  if t.peekBuf.len > 0: t.peekBuf.popFirst else: t.readNextToken
...
proc peekToken(p): Token =
  p.tokeniser.peekToken

proc eatToken(p): Token =
  p.tokeniser.eatToken
...
proc parseObject(p; allowImplicitBraces: bool = false): HoconNode =
  ...
  while true:
    ...
    of tkString:
      ...
      let node =
        if p.peekToken.kind == tkLeftBrace:
          p.parseObject
        else:
...
proc parseArray(p): HoconNode =
  ...
  while true:
    ...
    of tkRightBracket:
      ...
      let node = if p.peekToken.kind == tkLeftBrace: p.parseObject else: p.parseNode
...
proc write*(
  ...
  proc go(
      curr: HoconNode, parent: HoconNode, depth, indent: int, prevType: WrittenType
  ): WrittenType =
    var prevType = prevType

    case curr.kind
    of hnkArray:
      if prevType == wtFieldName:
        stream.write(" = ")
    ...
    of hnkObject:
      ...
      if writeBraces:
        if depth > 0 and prevType == wtFieldName:
          stream.write(" ")
        stream.write("{\n")
      ...
      for key, val in curr.fields:
        if val.kind == hnkObject:
          if (
            newlinesAroundObjects and prevType == wtObjectClose and
            depth <= newlinesAroundObjectsMaxDepth
          ) or (newlineAfterSimpleFields and prevType == wtSimpleField):
            stream.write("\n")
...
proc newHoconObject*(): HoconNode =
  HoconNode(kind: hnkObject)

proc newHoconArray*(): HoconNode =
  HoconNode(kind: hnkArray)

proc raiseHoconPathError*(path, message: string) {.noReturn.} =
  raise newException(HoconPathError, fmt"Invalid object path: {path}, {message}")

proc raiseHoconValueError*(src, target: HoconNodeKind, path: string) {.noReturn.} =
  raise newException(HoconValueError, fmt"Cannot read {src} as {target}, path: {path}")
...
proc getObject*(node: HoconNode, path: string): OrderedTable[string, HoconNode] =
  ...
  if n.kind == hnkObject:
    ...
    raiseHoconValueError(n.kind, hnkObject, path)
...
proc getArray*(node: HoconNode, path: string): seq[HoconNode] =
  ...
  if n.kind == hnkArray:
    ...
    raiseHoconValueError(n.kind, hnkArray, path)
...
proc set*(node: HoconNode, path: string, value: HoconNode, createPath = true) =
  ...
  for i, key {.inject.} in pathElems:
    ...
    if arrayIdx.isSome and arrayIdx.get < 0:
      raiseHoconPathError(path, fmt"Array index must be positive: {arrayIdx.get}")
...
"""
    ...
    assert t.eatToken ==
      Token(kind: tkString, str: "C:\\AUTOEXEC.BAT", line: 1, column: 1)
