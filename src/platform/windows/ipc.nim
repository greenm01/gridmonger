...
proc sendMessage(numBytes: int32) =
  discard WriteFile(g_pipe, g_buffer[0].addr, numBytes, nil, g_overlapped.addr)
...
proc tryRecv(): Option[AppEvent] =
  ...
  if ReadFile(g_pipe, g_buffer[0].addr, g_buffer.len.int32, nil, g_overlapped.addr) == 0:
    case GetLastError()
    of ErrorBrokenPipe:
  ...
  if GetOverlappedResult(g_pipe, g_overlapped.addr, numBytesTransferred.addr, false) == 0:
...
proc initServer*(): bool =
  ...
  g_pipe = CreateNamedPipe(
    ...
    1,
      # inbound buffer size - the kernel sets this automatically to
