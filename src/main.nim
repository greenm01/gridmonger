...
when not defined(DEBUG):
  const GridmongerMapFileFilter = "Gridmonger Map" & fmt" (*.{MapFileExt}):{MapFileExt}"
...
const
  ...
  FloorGroup1 = @[fDoor, fLockedDoor, fArchway]

  FloorGroup2 = @[fSecretDoor, fSecretDoorBlock, fOneWayDoor]

  FloorGroup3 = @[fPressurePlate, fHiddenPressurePlate]

  FloorGroup4 = @[fClosedPit, fOpenPit, fHiddenPit, fCeilingPit]

  FloorGroup5 = @[fTeleportSource, fTeleportDestination, fSpinner, fInvisibleBarrier]

  FloorGroup6 = @[fStairsDown, fStairsUp, fEntranceDoor, fExitDoor]

  FloorGroup7 = @[fBridge, fArrow]

  FloorGroup8 = @[fColumn, fStatue]
...
func mkQuickRefGeneral(a): seq[seq[QuickRefItem]] =
  @[
    ...
    @[
      scUndo.sc,
      "Undo last action".desc,
      scRedo.sc,
      "Redo last action".desc,
      @[scZoomIn, scZoomOut].sc(a = a),
      "Zoom in/out".desc,
      QuickRefSepa,
      scToggleWalkMode.sc,
      "Toggle walk mode".desc,
      scToggleWasdMode.sc,
      "Toggle WASD mode".desc,
      scToggleCellCoords.sc,
      "Toggle cell coordinates".desc,
      scToggleCurrentNotePane.sc,
      "Toggle current note pane".desc,
      scToggleNotesListPane.sc,
      "Toggle notes list pane".desc,
      scToggleToolsPane.sc,
      "Toggle tools pane".desc,
      scToggleTitleBar.sc,
      "Toggle title bar".desc,
      QuickRefSepa,
      scShowNoteTooltip.sc,
      "Show note tooltip".desc,
      scShowLinkLines.sc,
      "Show all link lines".desc,
      QuickRefSepa,
      scPreviousTheme.sc,
      "Previous theme".desc,
      scNextTheme.sc,
      "Next theme".desc,
      scReloadTheme.sc,
      "Reload current theme".desc,
      scToggleThemeEditor.sc,
      "Toggle theme editor".desc,
    ],
  ]
...
func mkQuickRefEditing(a): seq[seq[QuickRefItem]] =
  @[
    @[
      scExcavateTunnel.sc,
      "Excavate (draw) tunnel".desc,
      scEraseCell.sc,
      "Erase cell (clear floor & walls)".desc,
      scDrawClearFloor.sc,
      "Draw/clear floor".desc,
      ...
      QuickRefSepa,
      scDrawWall.sc,
      "Draw/clear wall".desc,
      scDrawSpecialWall.sc,
      "Draw/clear special wall".desc,
      @[scPreviousSpecialWall, scNextSpecialWall].sc(a = a),
      "Previous/next special wall".desc,
      QuickRefSepa,
      @[scPreviousFloorColor, scNextFloorColor].sc(a = a),
      "Previous/next floor colour".desc,
      scSetFloorColor.sc,
      "Set floor colour".desc,
      scPickFloorColor.sc,
      "Pick floor colour".desc,
      @[
    ...
    @[
      ...
      "Erase label".desc,
      QuickRefSepa,
      scJumpToLinkedCell.sc,
      "Jump to other side of link".desc,
      scLinkCell.sc,
...
func mkQuickRefInterface(a): seq[seq[QuickRefItem]] =
  @[
    ...
    @[
      ...
      "Restore window layout 4".desc, QuickRefSepa, scResetUIScaling.sc,
      "Reset interface scaling".desc,
    ],
  ]
...
func mkQuickRefShortcuts(a): seq[seq[seq[QuickRefItem]]] =
  @[mkQuickRefGeneral(a), mkQuickRefEditing(a), mkQuickRefInterface(a)]
...
const DiagonalMoveKeysCursor = DiagonalMoveKeys(
  upLeft: {keyY, keyKp7},
  upRight: {keyU, keyKp9},
...
let DefaultAppShortcuts = {
  ...
  scSelectionNone: @[mkKeyShortcut(keyU, {}), mkKeyShortcut(keyX, {})],
  scSelectionAddRect: @[mkKeyShortcut(keyR, {})],
  scSelectionSubRect: @[mkKeyShortcut(keyS, {})],
  scSelectionCopy: @[mkKeyShortcut(keyC, {}), mkKeyShortcut(keyY, {})],
...
proc setYubnAppShortcuts(sc: var Table[AppShortcut, seq[KeyShortcut]]) =
  ...
  sc[scUndo] = @[mkKeyShortcut(keyU, {mkCtrl}), mkKeyShortcut(keyZ, {mkCtrl})]
...
proc toStr(k: KeyShortcut): string =
  var s: seq[string] = @[]
  if mkSuper in k.mods:
    s.add("Cmd")
  elif mkCtrl in k.mods:
    s.add("Ctrl")

  if mkShift in k.mods:
    s.add("Shift")
  if mkAlt in k.mods:
    s.add("Alt")
...
proc rollLogFile(a) =
  ...
  let fileNames =
    @[p.logFile & ".bak3", p.logFile & ".bak2", p.logFile & ".bak1", p.logFile]
...
proc setSelectModeSelectMessage(a) =
  ...
  setStatusMessage(
    ...
    "Mark selection",
    @[
      scSelectionDraw.toStr(a),
      "draw",
      scSelectionErase.toStr(a),
      "erase",
      scSelectionAddRect.toStr(a),
      "add rect",
      scSelectionSubRect.toStr(a),
      "sub rect",
      scSelectionAll.toStr(a),
      "mark all",
      scSelectionNone.toStr(a),
      "unmark all",
      scSelectionCopy.toStr(a),
      "copy",
      special,
      "special",
    ],
...
proc setSelectModeSpecialActionsMessage(a) =
  setStatusMessage(
    ...
    "Mark selection",
    @[
      scSelectionEraseArea.toStr(a),
      "erase",
      scSelectionFillArea.toStr(a),
      "fill",
      scSelectionSurroundArea.toStr(a),
      "surround",
      scSelectionCropArea.toStr(a),
      "crop",
      scSelectionMove.toStr(a),
      "move",
      scSelectionSetFloorColorArea.toStr(a),
      "set colour",
    ],
    a,
  )
...
proc setSetLinkDestinationMessage(floor: Floor, a) =
  setStatusMessage(
    ...
    @[
      IconArrowsAll,
      "select cell",
      scAccept.toStr(a, idx = 0),
      "set",
      scCancel.toStr(a, idx = 0),
      "cancel",
    ],
    a,
  )
...
proc toolsPaneWidth(a): float =
  ...
  if a.layout.showToolsPane:
    if mainPane.h < ToolsPaneYBreakpoint2: ToolsPaneWidthWide else: ToolsPaneWidthNarrow
  else:
    0.0
...
proc toolsPaneHeight(mainPaneHeight: float): float =
  if mainPaneHeight < ToolsPaneYBreakpoint1:
    420.0
  elif mainPaneHeight < ToolsPaneYBreakpoint2:
    630.0
  else:
    780.0
...
proc calculateLevelDrawArea(a): tuple[w, h: float] =
  ...
  if a.layout.showCurrentNotePane:
    h -= CurrentNotePaneTopPad + CurrentNotePaneHeight + CurrentNotePaneBottomPad
...
proc stepCursor(cur: Location, dir: CardinalDir, steps: Natural, a): Location =
  ...
  of dirE:
    ...
    if viewCol > viewColMax:
      dp.viewStartCol =
        (l.cols - dp.viewCols).clamp(0, dp.viewStartCol + (viewCol - viewColMax))
  of dirS:
    ...
    if viewRow > viewRowMax:
      dp.viewStartRow =
        (l.rows - dp.viewRows).clamp(0, dp.viewStartRow + (viewRow - viewRowMax))
  of dirW:
...
proc moveCursorTo(loc: Location, a) =
  ...
  cur =
    if dx < 0:
      stepCursor(cur, dirW, -dx, a)
    elif dx > 0:
      stepCursor(cur, dirE, dx, a)
    else:
      cur

  cur =
    if dy < 0:
      stepCursor(cur, dirN, -dy, a)
    elif dy > 0:
      stepCursor(cur, dirS, dy, a)
    else:
      cur
...
proc stepLevelView(dir: CardinalDir, a) =
  ...
  case dir
  of dirE:
    newViewStartCol = (dp.viewStartCol + 1).clampMax(maxViewStartCol)
  of dirW:
    newViewStartCol = (dp.viewStartCol - 1).clampMin(0)
  of dirS:
    newViewStartRow = (dp.viewStartRow + 1).clampMax(maxViewStartRow)
  of dirN:
    newViewStartRow = (dp.viewStartRow - 1).clampMin(0)
...
# {{{ isKeyDown()
proc isKeyDown(
    ev: Event, keys: set[Key], mods: set[ModifierKey] = {}, repeat = false
): bool =
  let a =
    if repeat:
      {kaDown, kaRepeat}
...
proc isShortcutDown(
    ev: Event, shortcut: AppShortcut, a; repeat = false, ignoreMods = false
): bool =
...
func themePath(theme: ThemeName, a): string =
  let themeDir = if theme.userTheme: a.paths.userThemesDir else: a.paths.themesDir
...
proc makeUniqueThemeName(themeName: string, a): string =
  ...
  while true:
    ...
    if findThemeIndex(result, a).isNone:
      return
...
proc saveTheme(a) =
  try:
    ...
    setStatusMessage(IconFloppy, fmt"User theme '{a.currThemeName.name}' saved", a)
...
proc copyTheme(theme: ThemeName, newThemeName, newThemePath: string, a): bool =
  try:
    ...
    setStatusMessage(IconFloppy, fmt"User theme '{newThemeName}' created", a)
  except CatchableError as e:
...
proc renameTheme(theme: ThemeName, newThemeName, newThemePath: string, a): bool =
  try:
    ...
    setStatusMessage(IconFloppy, fmt"User theme renamed to '{newThemeName}'", a)
  except CatchableError as e:
...
proc loadThemeImage(imageName: string, userTheme: bool, a): Option[Paint] =
  if userTheme:
    ...
    if result.isNone:
      log.info(
        "Cannot load image from user theme images directory: " &
  ...
  result = loadImage(imgPath, a)
  if result.isNone:
    log.error("Cannot load image from default theme images directory: '{imgPath}'")
...
proc updateTheme(a) =
  ...
  a.theme.notesListPaneTheme =
    cfg.getObjectOrEmpty("pane.notes-list").toNotesListPaneTheme
...
proc restoreLayout(layout: Layout, a) =
  ...
  if layout.maximized: a.win.maximize else: a.win.unmaximize
...
proc saveAppConfig(cfg: HoconNode, path: string, a) =
  ...
  try:
    ...
    log.error(fmt"Cannot write config file '{path}'. Error message: {e.msg}")
  finally:
    a.logfile.flushFile
    if s != nil:
      s.close
...
proc saveAppConfig(a) =
  ...
  cfg.set(p & "editing.link-lines-mode", enumToDashCase($a.prefs.linkLinesMode))
...
proc saveMap(path: string, autosave, createBackup: bool, a) =
  ...
  try:
    ...
    let msgPrefix = if autosave: "Autosaving map failed" else: "Saving map failed"
    logError(e, msgPrefix)
    setErrorMessage(fmt"{msgPrefix}: {e.msg}", a)
...
template noteFields(dlgWidth: float) =
  ...
  ops.textArea(
    ...
    dlg.notes,
    activate = dlg.activateFirstTextField,
    constraint = TextAreaConstraint(maxLen: NotesLimits.maxRuneLen.some).some,
...
template validateLevelFields(dlg, map, validationError: untyped) =
  if dlg.locationName == "":
    ...
    for _, l in map.levels:
      if l.locationName == dlg.locationName and l.levelName == dlg.levelName and
...
func dialogButtonsStartPos(
    dlgWidth, dlgHeight: float, numButtons: Natural
): tuple[x, y: float] =
  const BorderPad = 15.0

  let x =
    dlgWidth - numButtons * DlgButtonWidth - BorderPad - (numButtons - 1) * DlgButtonPad
...
func moveGridPositionWrapping(
    currIdx: int, dc: int = 0, dr: int = 0, numItems, itemsPerRow: Natural
): Natural =
...
func handleGridRadioButton(
    ke: Event, currButtonIdx: Natural, numButtons, buttonsPerRow: Natural
): Natural =
  proc move(dc: int = 0, dr: int = 0): Natural =
    moveGridPositionWrapping(currButtonIdx, dc, dr, numButtons, buttonsPerRow)

  result =
    if ke.isKeyDown(MoveKeysStandard.left, repeat = true):
      move(dc = -1)
    elif ke.isKeyDown(MoveKeysStandard.right, repeat = true):
      move(dc = 1)
    elif ke.isKeyDown(MoveKeysStandard.up, repeat = true):
      move(dr = -1)
    elif ke.isKeyDown(MoveKeysStandard.down, repeat = true):
      move(dr = 1)
    else:
      currButtonIdx
...
# }}}
# {{{ handleTabNavigation()
proc handleTabNavigation(ke: Event, currTabIndex, maxTabIndex: Natural, a): Natural =
  result = currTabIndex

  if ke.isKeyDown(MoveKeysStandard.left, {mkCtrl}):
    if currTabIndex > 0:
      result = currTabIndex - 1
    else:
      result = maxTabIndex
    ...
    if ke.action == kaDown and mkCtrl in ke.mods and i >= 0 and i <= maxTabIndex:
      result = i

# }}}

# {{{ colorRadioButtonDrawProc()
proc colorRadioButtonDrawProc(
...
proc saveDiscardMapDialog(dlg: var SaveDiscardMapDialogParams, a) =
  ...
  if hasKeyEvent():
    ...
    if ke.isShortcutDown(scCancel, a):
      cancelAction(a)
    elif ke.isShortcutDown(scDiscard, a):
      discardAction(dlg, a)
    elif ke.isShortcutDown(scAccept, a):
      okAction(dlg, a)
    else:
...
proc editLevelPropsDialog(dlg: var LevelPropertiesDialogParams, a) =
  ...
  let l = currLevel(a)
  if dlg.locationName != l.locationName or dlg.levelName != l.levelName or
      dlg.elevation != $l.elevation:
    validateLevelFields(dlg, map, validationError)
...
proc resizeLevelDialog(dlg: var ResizeLevelDialogParams, a) =
  ...
  proc okAction(dlg: ResizeLevelDialogParams, a) =
    ...
    let newCur = actions.resizeLevel(
      a.doc.map, a.ui.cursor, newRows, newCols, align, a.doc.undoManager
    )
  ...
  try:
    sizeChanged = parseInt(dlg.rows) != l.rows or parseInt(dlg.cols) != l.cols
  except:
    discard
  ...
  if hasKeyEvent():
    ...
    if ke.isShortcutDown(scNextTextField, a):
      ...
      cancelAction(a)
    elif ke.isShortcutDown(scAccept, a) and sizeChanged:
      okAction(dlg, a)
    else:
      eventHandled = false
...
proc deleteLevelDialog(a) =
  ...
  ops.label(
    ...
    DlgWidth,
    h,
    "Do you want to delete the current level?",
    style = a.theme.labelStyle,
  )
...
proc editNoteDialog(dlg: var EditNoteDialogParams, a) =
  ...
  ops.textArea(
    ...
    dlg.text,
    activate = dlg.activateFirstTextField,
    constraint = TextAreaConstraint(maxLen: NoteTextLimits.maxRuneLen.some).some,
    style = a.theme.textAreaStyle,
  )
  ...
  proc okAction(dlg: EditNoteDialogParams, a) =
    ...
    var note = Annotation(kind: dlg.kind, text: dlg.text)
    case note.kind
    of akCustomId:
  ...
  if hasKeyEvent():
    ...
    of akIcon:
      dlg.icon = handleGridRadioButton(ke, dlg.icon, NoteIcons.len, IconsPerRow)
...
proc editLabelDialog(dlg: var EditLabelDialogParams, a) =
  ...
  ops.radioButtons(
    ...
    layout = RadioButtonsLayout(kind: rblGridHoriz, itemsPerRow: 4),
    drawProc = colorRadioButtonDrawProc(
      lt.labelTextColor.toSeq, a.theme.radioButtonStyle.buttonFillColorActive
    ).some,
    style = a.theme.radioButtonStyle,
  )
  ...
  if dlg.name == "":
    ...
    if dlg.name != currRegion(a).get.name:
      for name in l.regions.sortedRegionNames:
        if name == dlg.name:
          validationError =
            mkValidationError("A region already exists with the same name")
          break
...
proc openOverwriteThemeDialog(
    themeName: string, nextAction: proc(a: var AppContext), a
) =
...
proc copyThemeDialog(dlg: var CopyThemeDialogParams, a) =
  ...
  proc okAction(dlg: CopyThemeDialogParams, a) =
    closeDialog(a)
    let newThemePath = a.paths.userThemesDir / addFileExt(dlg.newThemeName, ThemeExt)
    proc copyTheme(a) =
...
proc redoAction(a) =
  ...
  if um.canRedo:
    ...
    setStatusMessage(IconRedo, fmt"Redid action: {undoStateData.actionName}", a)
  else:
...
proc startExcavateTunnelAction(a) =
  ...
  actions.excavateTunnel(
    ...
    undoLoc = cur,
    a.ui.currFloorColor,
    um = a.doc.undoManager,
    groupWithPrev = false,
  )

  setStatusMessage(IconPencil, "Excavate tunnel", @[IconArrowsAll, "excavate"], a)
...
proc doSetDrawWallActionRepeatMessage(name: string, a) =
  let icon =
    if a.ui.drawWallRepeatDirection.isHoriz: IconArrowsVert else: IconArrowsHoriz

  setStatusMessage(
    IconBorders, fmt"Draw {name} repeat", @[icon, mkRepeatWallActionString(name, a)], a
  )
...
proc selectPrevFloorColor(a) =
  if a.ui.currFloorColor > 0:
    dec(a.ui.currFloorColor)
  else:
    a.ui.currFloorColor = a.theme.levelTheme.floorBackgroundColor.high
...
proc handleLevelMouseEvents(a) =
  ...
  proc handlePanLevel(a) =
    ...
    let sensitivity = remap(
      inMin = MinZoomLevel,
      inMax = MaxZoomLevel,
      outMin = SensitivityMin,
      outMax = SensitivityMax,
...
proc showQuickReference(a) =
  ...
  setStatusMessage(
    ...
    @[
      fmt"Ctrl{HairSp}+{HairSp}{IconArrowsHoriz}",
      "switch tab",
      fmt"Esc{HairSp}/{HairSp}Space{HairSp}/{HairSp}Enter",
      "exit",
      "F1",
      "open user manual",
    ],
...
proc handleGlobalKeyEvents(a) =
  ...
  proc handleMoveWalk(ke: Event, a) =
    ...
    let
      ...
      isWasdKey = ke.key in AllWasdLetterKeys or ke.key in AllWasdKeypadKeys

      k = if ui.wasdMode: a.keys.walkKeysWasd else: a.keys.walkKeysCursor
    ...
    proc turnLeft(dir: CardinalDir): auto =
      dir.rotateACW

    proc turnRight(dir: CardinalDir): auto =
      dir.rotateCW

    template forward(): auto =
      ui.cursorOrient

    template backward(): auto =
      turnLeft(turnLeft(ui.cursorOrient))

    template left(): auto =
      turnLeft(ui.cursorOrient)

    template right(): auto =
      turnRight(ui.cursorOrient)

    template doAction(dir: CardinalDir, moveAction: bool) =
      if moveAction:
        if shiftDown:
          moveLevelView({dir}, s, a)
        else:
          moveCursor(dir, s, a)
      else:
        if not isRepeat:
  ...
  template moveKeyToCardinalDir(
      ke: Event, allowWasdKeys: bool, allowRepeat: bool
  ): Option[CardinalDir] =
    let k = if allowWasdKeys and ui.wasdMode: MoveKeysWasd else: MoveKeysStandard

    var kk = ke
    kk.mods = {}

    if kk.isKeyDown(k.left, repeat = allowRepeat):
      dirW.some
    elif kk.isKeyDown(k.right, repeat = allowRepeat):
      dirE.some
    elif kk.isKeyDown(k.up, repeat = allowRepeat):
      dirN.some
    elif kk.isKeyDown(k.down, repeat = allowRepeat):
      dirS.some
    else:
      CardinalDir.none
  ...
  # }}}
  # {{{ handleMoveKeys()
  template handleMoveKeys(
      ke: Event,
      allowWasdKeys, allowRepeat: bool,
      allowDiagonal: bool,
      moveHandler: untyped,
  ) =
    if allowDiagonal:
  ...
  # }}}
  # {{{ handleMoveCursor()
  proc handleMoveCursor(
      ke: Event, allowPan, allowJump, allowWasdKeys: bool, allowDiagonal: bool, a
  ): bool =
    if allowDiagonal:
      # Ignore Y/U/B/N keys if YUBN movement is not enabled in the prefs
      if not yubnMode and ke.key in DiagonalMoveLetterKeys:
    ...
    if allowJump and a.keys.primaryModKey in ke.mods:
      if ke.key in AllWasdLetterKeys:
        ...
        # shorcuts
        return
      elif a.prefs.modifierKeyMode == mkmCommandShift and ke.key in VimMoveKeys:
  ...
  proc drawWallRepeatMoveKeyHandler(dir: CardinalDir, mods: set[ModifierKey], a) =
    let cur = ui.cursor
    ...
    if dir.isHoriz == drawDir.isVert:
      ...
      if newCur != cur:
        if map.canSetWall(newCur, drawDir):
          ...
          setWarningMessage(
            "Cannot set wall of an empty cell", keepStatusMessage = true, a = a
          )
    else:
      let direction = if dir.isHoriz: "vertical" else: "horizontal"

      setWarningMessage(
        fmt"Can only repeat in {direction} direction", keepStatusMessage = true, a = a
      )
  ...
  if hasKeyEvent():
    ...
    of emNormal:
      ...
      if not ui.wasdMode and ke.isShortcutDown(scExcavateTunnel, a):
        ui.editMode = emExcavateTunnel
        startExcavateTunnelAction(a)
      elif not (ui.wasdMode and ui.walkMode) and ke.isShortcutDown(scEraseCell, a):
        ui.editMode = emEraseCell
        startEraseCellsAction(a)
      elif ke.isShortcutDown(scDrawClearFloor, a):
        ui.editMode = emDrawClearFloor
        setStatusMessage(
          IconEraser, "Draw/clear floor", @[IconArrowsAll, "draw/clear"], a
        )
        ...
        if floor in HorizVertFloors:
          ...
          let
            ...
            dir =
              if clockwise:
                map.getFloorOrientation(cur).rotateCW
              else:
                map.getFloorOrientation(cur).rotateACW
          ...
          setWarningMessage("Cannot change orientation of an empty cell", a = a)
        else:
          setWarningMessage("Cannot rotate floor", a = a)
      elif ke.isShortcutDown(scSetFloorColor, a):
        ui.editMode = emColorFloor
        setStatusMessage(
          IconBrush, "Set floor colour", @[IconArrowsAll, "set colour"], a
        )
      ...
      elif ke.isShortcutDown(scSelectSpecialWall1, a):
        selectSpecialWall(0, a)
      elif ke.isShortcutDown(scSelectSpecialWall2, a):
        selectSpecialWall(1, a)
      elif ke.isShortcutDown(scSelectSpecialWall3, a):
        selectSpecialWall(2, a)
      elif ke.isShortcutDown(scSelectSpecialWall4, a):
        selectSpecialWall(3, a)
      elif ke.isShortcutDown(scSelectSpecialWall5, a):
        selectSpecialWall(4, a)
      elif ke.isShortcutDown(scSelectSpecialWall6, a):
        selectSpecialWall(5, a)
      elif ke.isShortcutDown(scSelectSpecialWall7, a):
        selectSpecialWall(6, a)
      elif ke.isShortcutDown(scSelectSpecialWall8, a):
        selectSpecialWall(7, a)
      elif ke.isShortcutDown(scSelectSpecialWall9, a):
        selectSpecialWall(8, a)
      elif ke.isShortcutDown(scSelectSpecialWall10, a):
        selectSpecialWall(9, a)
      elif ke.isShortcutDown(scSelectSpecialWall11, a):
        selectSpecialWall(10, a)
      elif ke.isShortcutDown(scSelectSpecialWall12, a):
        selectSpecialWall(11, a)
      elif ke.isShortcutDown(scPreviousSpecialWall, repeat = true, a = a):
        if ui.currSpecialWall > 0:
          dec(ui.currSpecialWall)
        else:
          ui.currSpecialWall = SpecialWalls.high
      elif ke.isShortcutDown(scNextSpecialWall, repeat = true, a = a):
        if ui.currSpecialWall < SpecialWalls.high:
          inc(ui.currSpecialWall)
        else:
          ui.currSpecialWall = 0
      ...
      elif ke.isShortcutDown(scPickFloorColor, a):
        pickFloorColor(a)
      elif ke.isShortcutDown(scSelectFloorColor1, a):
        selectFloorColor(0, a)
      elif ke.isShortcutDown(scSelectFloorColor2, a):
        selectFloorColor(1, a)
      elif ke.isShortcutDown(scSelectFloorColor3, a):
        selectFloorColor(2, a)
      elif ke.isShortcutDown(scSelectFloorColor4, a):
        selectFloorColor(3, a)
      elif ke.isShortcutDown(scSelectFloorColor5, a):
        selectFloorColor(4, a)
      elif ke.isShortcutDown(scSelectFloorColor6, a):
        selectFloorColor(5, a)
      elif ke.isShortcutDown(scSelectFloorColor7, a):
        selectFloorColor(6, a)
      elif ke.isShortcutDown(scSelectFloorColor8, a):
        selectFloorColor(7, a)
      elif ke.isShortcutDown(scSelectFloorColor9, a):
        selectFloorColor(8, a)
      elif ke.isShortcutDown(scSelectFloorColor10, a):
        selectFloorColor(9, a)
      elif ke.isShortcutDown(scUndo, repeat = true, a = a):
        undoAction(a)
      elif ke.isShortcutDown(scRedo, repeat = true, a = a):
        redoAction(a)
      ...
      #          setWarningMessage("Not a linked cell", a=a)
        ...
        zoomIn(a)
        setStatusMessage(IconZoomIn, fmt"Zoomed in – level {dp.getZoomLevel}", a)
      elif ke.isShortcutDown(scZoomOut, repeat = true, a = a):
        zoomOut(a)
        setStatusMessage(IconZoomOut, fmt"Zoomed out – level {dp.getZoomLevel}", a)
      ...
      elif ke.isShortcutDown(scEditPreferences, a):
        ...
        if map.levels.len < NumLevelsLimits.maxInt:
          ...
          setWarningMessage(
            "Cannot add new level: maximum number of levels has been reached " &
              fmt"({NumLevelsLimits.maxInt})",
            a = a,
          )
      elif ke.isShortcutDown(scDeleteLevel, a):
        openDeleteLevelDialog(a)
      elif ke.isShortcutDown(scNewMap, a):
        newMap(a)
      elif ke.isShortcutDown(scEditMapProps, a):
        ...
        if l.regionOpts.enabled:
          ...
          setWarningMessage(
            "Cannot edit region properties: regions are not enabled for level", a = a
          )
      elif ke.isShortcutDown(scOpenMap, a):
        openMap(a)
      elif ke.isShortcutDown(scSaveMap, a):
        saveMap(a)
      elif ke.isShortcutDown(scSaveMapAs, a):
        saveMapAs(a)
      elif ke.isShortcutDown(scReloadTheme, a):
        reloadTheme(a)
      elif ke.isShortcutDown(scPreviousTheme, a):
        selectPrevTheme(a)
      elif ke.isShortcutDown(scNextTheme, a):
        selectNextTheme(a)
      ...
      # Save/restore layout
      elif ke.isShortcutDown(scSaveLayout1, a):
        saveLayout(0, a)
      elif ke.isShortcutDown(scSaveLayout2, a):
        saveLayout(1, a)
      elif ke.isShortcutDown(scSaveLayout3, a):
        saveLayout(2, a)
      elif ke.isShortcutDown(scSaveLayout4, a):
        saveLayout(3, a)
      elif ke.isShortcutDown(scRestoreLayout1, a):
        restoreLayout(0, a)
      elif ke.isShortcutDown(scRestoreLayout2, a):
        restoreLayout(1, a)
      elif ke.isShortcutDown(scRestoreLayout3, a):
        restoreLayout(2, a)
      elif ke.isShortcutDown(scRestoreLayout4, a):
        restoreLayout(3, a)
    ...
    of emExcavateTunnel, emEraseCell, emEraseTrail, emDrawClearFloor, emColorFloor:
      ...
      if ke.isShortcutUp(
        {scEraseCell, scDrawClearFloor, scEraseTrail, scSetFloorColor}, a
      ):
        ui.editMode = emNormal
        clearStatusMessage(a)
    ...
    of emDrawWall:
      proc handleMoveKey(dir: CardinalDir, mods: set[ModifierKey], a) =
        ...
        if map.canSetWall(cur, dir):
          let w = if map.getWall(cur, dir) == wWall: wNone else: wWall
          ...
          actions.setWall(
            map, loc = cur, undoLoc = cur, dir, w, um, groupWithPrev = false
          )
      ...
      if not ui.wasdMode and ke.isShortcutUp(scDrawWall, a):
        ...
        if ui.drawWallRepeatAction == dwaNone:
          setWarningMessage(
            "Set or clear wall in current cell first", keepStatusMessage = true, a = a
          )
        else:
      ...
      proc handleMoveKey(dir: CardinalDir, mods: set[ModifierKey], a) =
        ...
        if map.canSetWall(cur, dir):
          ...
          if curSpecWall == wOneWayDoorNE:
            if dir in {dirS, dirW}:
              curSpecWall = wOneWayDoorSW
          elif curSpecWall == wLeverSW:
            if dir in {dirN, dirE}:
              curSpecWall = wLeverNE
          elif curSpecWall == wNicheSW:
            if dir in {dirN, dirE}:
              curSpecWall = wNicheNE
          elif curSpecWall == wStatueSw:
            if dir in {dirN, dirE}:
              curSpecWall = wStatueNE
    ...
    of emSelect:
      ...
      if primaryModDown(a):
        setSelectModeSpecialActionsMessage(a)
      else:
        setSelectModeSelectMessage(a)
      ...
      if ke.isShortcutDown(scSelectionDraw, a):
      ...
      elif ke.isShortcutDown(scSelectionAll, a):
        ui.selection.get.fill(true)
        ...
        if bbox.isSome:
          let bbox = bbox.get
          var bboxTopLeft = Location(levelId: cur.levelId, col: bbox.c1, row: bbox.r1)
        ...
        if bbox.isSome:
          actions.fillSelection(
            map, cur.levelId, selection, bbox.get, ui.currFloorColor, um
          )
          exitSelectMode(a)
        ...
        let bbox = selection.boundingBox
        if bbox.isSome:
          actions.setSelectionFloorColor(
            map, cur.levelId, selection, bbox.get, ui.currFloorColor, um
          )
          exitSelectMode(a)
    ...
    of emNudgePreview:
      proc handleMoveKey(dir: CardinalDir, mods: set[ModifierKey], a) =
        ...
        case dir
        of dirE:
          dp.selStartCol = (dp.selStartCol + step).clampMax(cols - 1)
        of dirS:
          dp.selStartRow = (dp.selStartRow + step).clampMax(rows - 1)
        of dirW:
          dp.selStartCol = (dp.selStartCol - step).clampMin(-cols + 1)
        of dirN:
          dp.selStartRow = (dp.selStartRow - step).clampMin(-rows + 1)
    ...
    of emSetCellLink:
      ...
      if ke.isShortcutDown(scAccept, a):
        if map.isEmpty(cur):
          setWarningMessage(
            "Cannot set link destination to an empty cell",
            keepStatusMessage = true,
            a = a,
          )
        elif cur == ui.linkSrcLocation:
          setWarningMessage(
            "Cannot set link destination to the source cell",
            keepStatusMessage = true,
            a = a,
          )
        else:
          actions.setLink(
            map, src = ui.linkSrcLocation, dest = cur, ui.currFloorColor, um
          )
...
proc handleQuickRefKeyEvents(a) =
  if hasKeyEvent():
    ...
    if ke.isShortcutDown(scReloadTheme, a):
      reloadTheme(a)
    elif ke.isShortcutDown(scPreviousTheme, a):
      selectPrevTheme(a)
    elif ke.isShortcutDown(scNextTheme, a):
...
proc renderRegionDropDown(a) =
  ...
  if currRegion.isSome:
    ...
    let regionDropDownWidth = round(
      a.vg.textWidth(currRegionName) + a.theme.levelDropDownStyle.label.padHoriz * 2 +
        8.0
    )
    ...
    if sortedRegionIdx != prevSortedRegionIdx:
      ...
      let (r, c) = a.doc.map.getRegionCenterLocation(ui.cursor.levelId, regionCoords)
...
# }}}
# {{{ renderNoteTooltip()
proc renderNoteTooltip(
    x, y: float, levelDrawWidth, levelDrawHeight: float, note: Annotation, a
) =
  alias(vg, a.vg)
  alias(ui, a.ui)
  alias(dp, a.ui.drawLevelParams)
  ...
  if note.text != "":
    ...
    vg.beginPath
    vg.roundedRect(
      noteBoxX, noteBoxY, noteBoxW, noteBoxH, radius = lt.noteTooltipCornerRadius
    )
    vg.fill
...
# {{{ renderLevel()
proc renderLevel(x, y, w, h: float, levelDrawWidth, levelDrawHeight: float, a) =
  alias(dp, a.ui.drawLevelParams)
  alias(ui, a.ui)
  alias(opts, a.opts)
    ...
    dp.selectionBuffer = (
      if ui.editMode == emPastePreview or
        ...
        (
          ...
          ui.prevEditMode in {emMovePreview, emNudgePreview}
        ): ui.nudgeBuf
...
proc specialWallDrawProc(
  ...
  return proc(
    ...
    of wNone:
      discard
    of wWall:
      drawSolidWallHoriz(cx, cy, ot, ctx = ctx)
    of wIllusoryWall:
      drawIllusoryWallHoriz(cx + 2, cy, ot, ctx = ctx)
    of wInvisibleWall:
      drawInvisibleWallHoriz(cx - 2, cy, ot, ctx = ctx)
    of wDoor:
      drawDoorHoriz(cx, cy, ot, ctx = ctx)
    of wLockedDoor:
      drawLockedDoorHoriz(cx, cy, ot, ctx = ctx)
    of wArchway:
      drawArchwayHoriz(cx, cy, ot, ctx = ctx)
    of wSecretDoor:
      drawAtZoomLevel(6):
        drawSecretDoorHoriz(cx - 2, cy, ot, ctx = ctx)
    of wOneWayDoorNE:
      drawAtZoomLevel(8):
        drawOneWayDoorHorizNE(cx - 4, cy + 1, ot, ctx = ctx)
    of wLeverSW:
      drawAtZoomLevel(6):
        drawLeverHorizSW(cx - 2, cy + 1, ot, ctx = ctx)
    of wNicheSW:
      drawNicheHorizSW(cx, cy, ot, floorColor = 0, ctx = ctx)
    of wStatueSW:
      drawAtZoomLevel(6):
        drawStatueHorizSW(cx - 2, cy + 2, ot, ctx = ctx)
    of wKeyhole:
      drawAtZoomLevel(6):
        drawKeyholeHoriz(cx - 2, cy, ot, ctx = ctx)
    of wWritingSW:
      drawAtZoomLevel(12):
        drawWritingHorizSW(cx - 6, cy + 4, ot, ctx = ctx)
    else:
      discard
...
# {{{ renderIndexedNote()
proc renderIndexedNote(
    x, y: float,
    size: float,
    bgColor, fgColor: Color,
    shape: NoteBackgroundShape,
    index: Natural,
    a;
) =
  alias(vg, a.vg)
...
# }}}
# {{{ renderNoteMarker()
proc renderNoteMarker(
    x, y, w, h: float,
    note: Annotation,
    textColor: Color,
    indexedNoteSize: float = 36.0,
    a;
) =
  alias(vg, a.vg)
...
proc renderCurrentNotePane(x, y, w, h: float, a) =
  ...
  if note.isSome and not (a.ui.editMode in {emPastePreview, emNudgePreview}):
    let note = note.get
    if note.text == "" or note.kind == akLabel:
      return

    renderNoteMarker(
      x, y, w, h, note, textColor = a.theme.currentNotePaneTheme.textColor, a = a
    )
...
func sortByTextAndLocation(
    locX: Location, x: Annotation, locY: Location, y: Annotation
): int =
  var c = cmpNaturalIgnoreCase(x.text.toRunes, y.text.toRunes)
  if c != 0:
    return c
  c = cmp(locX.levelId, locY.levelId)
  if c != 0:
    return c
  c = cmp(locX.row, locY.row)
  if c != 0:
    return c
...
proc rebuildNotesListCache(textW: float, a) =
  ...
  func sortByTextLocationType(x, y: NotesListCacheEntry): int =
    ...
    var c = sortByTextAndLocation(x.location, noteX, y.location, noteY)
    if c != 0:
      return c
  ...
  func sortByTypeTextLocation(x, y: NotesListCacheEntry): int =
    ...
    if c != 0:
      return c
    sortByTextAndLocation(x.location, noteX, y.location, noteY)
  ...
  proc toAnnotationKindSet(filter: set[NoteTypeFilter]): set[AnnotationKind] =
    for f in filter:
      case f
      of ntfNone:
        result.incl(akComment)
      of ntfNumber:
        result.incl(akIndexed)
      of ntfId:
        result.incl(akCustomId)
      of ntfIcon:
        result.incl(akIcon)
  ...
  proc collectLevelNotes(l: Level, a): auto =
    ...
    for r, c, note in l.allNotes:
      s.maybeAddCacheEntry(Location(levelId: l.id, row: r, col: c), note, vg)
  ...
  proc collectRegionNotes(l: Level, rc: RegionCoords, a): auto =
    ...
    for loc, note in map.regionNotes(l.id, rc):
      s.maybeAddCacheEntry(
        Location(levelId: l.id, row: loc.row, col: loc.col), note, vg
      )
    sortCacheEntries(s, nls.currFilter.orderBy)
    s
...
# {{{ noteButton()
proc noteButton(
    id: ItemId, textX, textY, textW, markerX: float, note: Annotation, active: bool
): bool =
  alias(ui, g_app.ui)
  ...
  ops.addLayoutDrawLayer(ops.currentLayer(), slot.nodeId, vg, bounds):
    ...
    if active or hover:
      let bgColor =
        if active: nt.itemBackgroundActiveColor else: nt.itemBackgroundHoverColor
    ...
    let textColor =
      if active:
        nt.itemTextActiveColor
      elif hover:
        nt.itemTextHoverColor
      else:
        nt.itemTextNormalColor

    if note.kind == akIndexed:
      renderNoteMarker(
        x + markerX + 3, y + 2, w, h, note, textColor, indexedNoteSize = 32, g_app
      )
    else:
...
proc renderNotesListPane(x, y, w, h: float, a) =
  ...
  let expandCollapseDisabled =
    case nls.currFilter.scope
    of nsfMap:
      false
    of nsfLevel:
      not l.regionOpts.enabled
    of nsfRegion:
      true
  ...
  for e in nls.cache:
    ...
    of nckNote:
      ...
      if noteButton(
        ...
        markerX,
        note,
        active = (nls.activeId.isSome and nls.activeId.get == e.id),
      ):
...
proc renderThemeEditorProps(x, y, w, h: float, a) =
  ...
  if ops.sectionHeader("Level", te.sectionLevel):
    ...
    if ops.subSectionHeader("Notes", te.sectionNotes):
      ...
      group:
        enumProp("Background Shape", p & "background-shape", NoteBackgroundShape)
...
# {{{ renderCommand()
proc renderCommand(
    x, y: float, command: string, bgColor, textColor: Color, a: AppContext
): float =
  alias(vg, a.vg)
...
proc renderQuickReference(x, y, w, h: float, a) =
  ...
  proc renderSection(
      x, y: float, items: seq[QuickRefItem], colWidth: float, a: AppContext
  ) =
    const
      RowHeight = 24.0
      SepaHeight = 14.0
  ...
  let (viewHeight, col1Width, col2Width) =
    case a.quickRef.activeTab
    of 0:
      (520.0, DefaultColWidth, DefaultColWidth)
    of 1:
      (655.0, DefaultColWidth, DefaultColWidth)
    else:
...
proc renderUI(a) =
  ...
  if a.ui.showQuickReference:
    ...
    if not map.hasLevels:
      ...
      renderModeAndOptionIndicators(
        x = mainPane.x1 + LevelLeftPad_NoCoords, y = a.win.titleBarHeight + 32, a
      )
    ...
    if map.hasLevels:
      if a.layout.showCurrentNotePane:
        var paneWidth = mainPane.w - CurrentNotePaneLeftPad - CurrentNotePaneRightPad
      ...
      if a.layout.showNotesListPane:
        renderNotesListPane(
          x = 0, y = mainPane.y1, w = NotesListPaneWidth, h = mainPane.h, a
        )
...
proc renderFrameCb(a) =
  ...
  if a.useMainWindowSplash():
    ...
    elif a.doc.map.hasLevels:
      handleGlobalKeyEvents(a)
    else:
...
proc initPreferences(cfg: HoconNode, a) =
  ...
  with a.prefs:
    ...
    splashTimeoutSecs = prefs
      .getNaturalOrDefault("splash.auto-close.timeout-secs", 3)
    ...
    scaleFactor =
      prefs
      .getIntOrDefault("interface.scale-percentage", 100)
    ...
    if prefs.getOpt("modifier-key-mode").isSome:
      modifierKeyMode = prefs.getEnumOrDefault("modifier-key-mode", ModifierKeyMode)
      when not defined(macosx):
        # Revert it to Ctrl+Alt if using a config copied from a Mac
        modifierKeyMode = mkmControlAlt
    else:
      modifierKeyMode = when defined(macosx): mkmCommandShift else: mkmControlAlt
    ...
    walkCursorMode = prefs.getEnumOrDefault("editing.walk-cursor-mode", WalkCursorMode)

    openEndedExcavate = prefs.getBoolOrDefault("editing.open-ended-excavate")

    linkLinesMode = prefs.getEnumOrDefault("editing.link-lines-mode", LinkLinesMode)
...
proc restoreLayoutsFromConfig(cfg: HoconNode, a) =
  proc toLayout(cfg: HoconNode): Layout =
    var l = Layout(
      showCurrentNotePane: cfg.getBoolOrDefault("show-current-note-pane", true),
...
proc applyWindowConfigOverrides(cfg: WindowConfig, a) =
  ...
  let (x, y) = a.win.pos
  if cfg.x.isSome:
    a.win.pos = (cfg.x.get, y)
  if cfg.y.isSome:
    a.win.pos = (x, cfg.y.get)

  let (width, height) = a.win.size
  if cfg.width.isSome:
    a.win.size = (cfg.width.get, height)
  if cfg.height.isSome:
    a.win.size = (width, cfg.height.get)

  if cfg.maximized.isSome:
    if cfg.maximized.get: a.win.maximize else: a.win.unmaximize
...
proc initApp(
  ...
  let mapFileName =
    if mapFile.isSome:
      ...
      cfg.getStringOrDefault("last-state.last-document", "")
  ...
  # Init window
  a.win.renderFramePreCb = proc(win: CSDWindow) =
    renderFramePreCb(g_app)
  ...
  proc crashHandler(e: ref Exception, a) =
    ...
    if doAutoSave:
      if crashAutosavePath == "":
        ...
        msg &= "The map has been autosaved as '" & crashAutosavePath
...
proc main() =
  ...
  try:
    ...
    log.info(
      fmt"Command line parameters: configFile: {configFile}, " &
    ...
    while not a.shouldClose:
      ...
      if event.isSome:
        ...
        case event.kind
        of aeFocus:
          handleFocusEvent(event, a)
        of aeOpenFile:
          handleOpenFileEvent(event, a)
        of aeAutoSave:
          handleAutoSaveEvent(event, a)
        of aeVersionUpdate:
