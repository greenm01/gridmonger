...
proc boundingBox*(s): Option[Rect[Natural]] =
  ...
  while r1 < s.rows and isRowEmpty(r1):
    inc(r1)

  if r1 < s.rows:
    while c1 < s.cols and isColEmpty(c1):
      inc(c1)
    while c2 > 0 and isColEmpty(c2):
      dec(c2)
    while r2 > 0 and isRowEmpty(r2):
      dec(r2)
...
proc isNeighbourCellEmpty*(s; r, c: Natural, dir: CardinalDir): bool =
  ...
  case dir
  of dirN:
    result = r == 0 or not s[r - 1, c]
  of dirE:
    result = c == s.cols - 1 or not s[r, c + 1]
  of dirS:
    result = r == s.rows - 1 or not s[r + 1, c]
  of dirW:
    result = c == 0 or not s[r, c - 1]
